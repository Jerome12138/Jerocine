...
```text
deploy/
├─ docker-compose.yml   # 服务编排: mysql / redis / migrate / film / nginx (build.context = 仓库根)
...
1. 准备密钥（RS256，挂载给 distroless 容器）：
   ...
   ⚠️ `secrets/` 与 `apk/` 目录及其内容必须属 **65532:65532**（distroless nonroot UID），否则 film_api 读不到密钥而启动失败。
...
3. 启动（在 `deploy/` 目录）：
   ...
   启动顺序：mysql/redis → `migrate` 一次性服务跑完 `server/migrations/` → film_api → nginx。
...
```bash
# 只更新前端（--no-deps 不会连带重启 film_api，不打断采集任务）
sudo docker compose build nginx && sudo docker compose up -d --no-deps nginx

# 更新后端（会重启 film_api；若有采集任务先 POST /api/v1/manage/spider/jobs/:id/pause）
sudo docker compose build film && sudo docker compose up -d film   # 会先自动跑待应用迁移
...
sudo docker exec film_nginx sh -c "rm -rf /var/cache/nginx/api/* && nginx -s reload"

# 2. Redis 应用层缓存（v1:movie:detail:* 等，带 TTL）
sudo docker exec film_redis redis-cli -a "$REDIS_PASSWORD" --no-auth-warning \
  --scan --pattern 'v1:movie:detail:*' | xargs -r docker exec -i film_redis redis-cli -a "$REDIS_PASSWORD" --no-auth-warning del
```

## 健康检查与排障

- 容器健康：`sudo docker compose ps`（全部应为 healthy；film_api 内置 `-healthcheck` 子命令）
- 冒烟：`curl localhost:8080/`（前端 200）、`curl localhost:8080/api/v1/films?keyword=test`（API 200）
- 日志：`sudo docker logs film_api`、`sudo docker logs film_nginx`
