...
describe('ManageSidebar variant', () => {
  ...
  it('variant=drawer + open=true → 渲染 drawer 全屏 root', () => {
    ...
    expect(document.querySelector('.jc-drawer-root')).toBeTruthy()
    expect(document.querySelector('.jc-drawer-panel')).toBeTruthy()
    expect(document.querySelector('.jc-drawer-mask')).toBeTruthy()
  })

  it('variant=drawer + open=false → 不渲染 drawer', () => {
    mountSidebar({ variant: 'drawer', open: false })
    expect(document.querySelector('.jc-drawer-root')).toBeNull()
  })
  ...
  it('variant=mini → aside style width 72px, 且菜单项带可见中文标签(触屏无 hover 可辨识)', () => {
    ...
    const labels = aside.querySelectorAll('.jc-ms__mini-label')
  ...
  it('drawer 模式 panel 宽度 260px', () => {
    mountSidebar({ variant: 'drawer', open: true })
    const panel = document.querySelector('.jc-drawer-panel') as HTMLElement
  ...
  it('drawer 模式点击遮罩 emit close', async () => {
    ...
    const mask = document.querySelector('.jc-drawer-mask') as HTMLElement
  ...
  it('drawer 模式点击菜单 item emit close', async () => {
    const w = mountSidebar({ variant: 'drawer', open: true })
    const link = document.querySelector('.jc-drawer-root a') as HTMLElement
