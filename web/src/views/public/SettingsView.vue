 ...
 *   - 检查更新 (仅 APK)
 *   - 更换服务器地址 (仅 APK)
 *   - 广告过滤默认开关 (web 端, 复用 PlayView 的 jc-ad-filter 持久化键)
 *
 * 渲染分支:
 *   - 桌面 / 移动: 原有卡片式布局 (保持不变)
...
/**
 * 广告过滤默认开关 — 复用 PlayView 的 localStorage 键 jc-ad-filter,
...
<template>
  ...
  <main v-if="isTV" class="jc-set-tv">
    ...
    <div class="jc-set-tv__split">
      <!-- 左: 设置分组列表 -->
      <nav class="jc-set-tv__groups">
        <a
          v-for="g in groups"
          :key="g.id"
          class="jc-tv-listrow jc-set-tv__group"
        ...
        >
          <span class="jc-set-tv__group-ic">{{ g.icon }}</span>
      ...
      <!-- 右: 当前组设置项面板 -->
      <div class="jc-tv-panel" :class="activeGroup === 'about' ? 'z3' : activeGroup === 'account' ? 'z2' : 'z1'">
        <!-- 播放 -->
        <template v-if="activeGroup === 'play'">
          ...
          <div class="jc-set-tv__rows">
            ...
            <div class="jc-tv-listrow jc-set-tv__note">
        ...
        <template v-else-if="activeGroup === 'adfilter'">
          ...
          <div class="jc-set-tv__rows">
            ...
            >
              ...
              <span class="right">
                <span class="jc-tv-toggle" :class="{ on: adFilter }"><i></i></span>
              </span>
            </div>
            <div class="jc-tv-listrow jc-set-tv__note">
              <span class="jc-set-tv__note-ic">🛡</span>
        ...
        <template v-else-if="activeGroup === 'account'">
          ...
          <div class="jc-set-tv__rows">
            ...
            >
              ...
              <span class="right"><span class="jc-tv-btn">退出</span></span>
            </div>
          </div>
        </template>
        ...
        <template v-else-if="activeGroup === 'device'">
          ...
          <div class="jc-set-tv__rows">
            ...
            <div class="jc-tv-listrow">
              ...
              <span class="right jc-set-tv__mono">{{ serverOrigin }}</span>
            </div>
            <div
              class="jc-tv-listrow"
              data-focusable="true"
            ...
            >
              ...
              <span class="right"><span class="jc-tv-btn cyan">更换</span></span>
            </div>
          </div>
        </template>
        ...
        <template v-else>
          ...
          <div class="jc-set-tv__rows">
            ...
            <div class="jc-tv-listrow">
              ...
              <span class="right jc-set-tv__mono">{{ buildMode }}</span>
            </div>
            <div
              class="jc-tv-listrow"
              data-focusable="true"
            ...
            >
              ...
              <span class="right">
                <span class="jc-tv-btn primary">{{ checking ? '检查中…' : '立即检查' }}</span>
              </span>
            </div>
          </div>
        </template>
  ...
  <!-- ============ 桌面 / 移动: 原有布局 ============ -->
  <main v-else class="container-page py-[var(--jc-space-6)]">
    <h1 class="text-2xl font-[var(--jc-fw-semibold)] mb-[var(--jc-space-5)]">设置</h1>

    <section class="jc-settings-card">
      <h2 class="jc-settings-card__title">应用信息</h2>
      <div class="jc-settings-row">
        <span class="jc-settings-row__label">运行平台</span>
        <span class="jc-settings-row__value">{{ isNative ? 'Jerocine影视 TV APP' : 'Web 浏览器' }}</span>
      </div>
      <div class="jc-settings-row">
        <span class="jc-settings-row__label">服务器地址</span>
        <span class="jc-settings-row__value font-mono break-all">{{ serverOrigin }}</span>
      </div>
      <div class="jc-settings-row">
        <span class="jc-settings-row__label">构建模式</span>
        <span class="jc-settings-row__value font-mono">{{ buildMode }}</span>
      </div>
    </section>

    <section v-if="isNative" class="jc-settings-card">
      <h2 class="jc-settings-card__title">应用更新</h2>
      <div class="jc-settings-row">
        <span class="jc-settings-row__label">检查新版本</span>
...
.jc-settings-row__label {
  font-size: var(--jc-fs-sm);
  color: var(--jc-text-secondary);
  flex-shrink: 0;
}
.jc-settings-row__value {
  font-size: var(--jc-fs-sm);
  color: var(--jc-text-primary);
  text-align: right;
}
...
/* ============ TV 雷鸟分屏 ============ */
.jc-set-tv {
  padding: var(--jc-space-4) var(--jc-space-6) var(--jc-space-8);
}
.jc-set-tv__split {
  display: grid;
...
.jc-set-tv__group.is-active {
  background: var(--jc-brand-gradient);
  border-color: transparent;
}
.jc-set-tv__group.is-active .right {
