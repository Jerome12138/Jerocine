...
public class PlayerActivity extends android.app.Activity {
    ...
    private static final String TAG = "JerocinePlayer";
    ...
    private final Set<Integer> forceRelayIdx = new HashSet<>();// 直连分片失败后强制中转的集(切源重置)
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        playerView.setControllerVisibilityListener((PlayerView.ControllerVisibilityListener) v -> {
            boolean visible = v == View.VISIBLE;
            setControllerChromeVisible(titleBar, visible);
            setControllerChromeVisible(controlsRoot, visible);
            if (v == View.VISIBLE) {
                bindControlButtons();
                playerView.post(() -> {
    ...
    private void setControllerChromeVisible(View chrome, boolean visible) {
        ...
        if ("on".equals(ServiceLocator.reduceMotionMode())) {
            chrome.setAlpha(1f);
        } else {
            chrome.setAlpha(0f);
            chrome.animate().alpha(1f).setDuration(140L).start();
        }
    }
    ...
    private void initPlayer() {
        ...
        player.addListener(new Player.Listener() {
            ...
            public void onPlayerError(@androidx.annotation.NonNull androidx.media3.common.PlaybackException error) {
                ...
                if (adFilterOn && shouldRetryWithRelay(currentUrl, failedUrl)
                        && errIdx >= 0 && errIdx < currentRawUrls.size()
                        && !forceRelayIdx.contains(errIdx)) {
                    Log.w(TAG, "Direct media request failed, retrying through relay. failedUrl="
                            + failedUrl, error);
                    forceRelayIdx.add(errIdx);
                    forceRawIdx.remove(errIdx);
                    sLastPlaybackFailed = false;
                    retryCurrentItem(errIdx, "直连失败, 已切换中转");
                    return;
                }
                // 服务端无法抓取清单时，仅本集回退原始源并在设备侧过滤。
                if (adFilterOn && currentUrl.contains("/m3u8/proxy")
                        && !currentUrl.toLowerCase(java.util.Locale.US).contains("proxymedia=1")
                        && (failedUrl.isEmpty() || failedUrl.contains("/m3u8/proxy"))
                        && errIdx >= 0 && errIdx < currentRawUrls.size()
                        && !forceRawIdx.contains(errIdx)) {
                    Log.w(TAG, "Proxy playback failed, retrying raw URL. url=" + currentUrl, error);
                    forceRawIdx.add(errIdx);
                    forceRelayIdx.remove(errIdx);
                    sLastPlaybackFailed = false;       // 正在自愈, 不当致命失败
                    retryCurrentItem(errIdx, "清单代理失败, 已切换直连");
                    return;
                }
                Log.e(TAG, "Playback failed. code=" + error.errorCode
                        + " name=" + error.getErrorCodeName() + " url=" + currentUrl, error);
    ...
    /** 默认仅由服务端处理清单，媒体分片由设备直连；兼容失败时按集切到全量中转。 */
    private String mediaUriFor(int idx, String rawUrl) {
        return buildPlayableUrl(idx, rawUrl, adFilterOn, forceRawIdx.contains(idx),
                forceRelayIdx.contains(idx), proxyBase);
    }
    ...
    static String buildPlayableUrl(
        ...
        String lower = rawUrl.toLowerCase(java.util.Locale.US);
        boolean isM3u8 = lower.endsWith(".m3u8")
                || lower.contains(".m3u8?")
                || lower.contains(".m3u8#");
        if (!isM3u8 || lower.contains("/m3u8/proxy?")) return rawUrl;
        try {
    ...
    static boolean shouldRetryWithRelay(String currentMediaUrl, String failedRequestUrl) {
        if (currentMediaUrl == null || failedRequestUrl == null) return false;
        String current = currentMediaUrl.toLowerCase(java.util.Locale.US);
        String failed = failedRequestUrl.toLowerCase(java.util.Locale.US);
    ...
    private void retryCurrentItem(int idx, String message) {
        final long pos = Math.max(0, player.getCurrentPosition());
        outroHandler.post(() -> {
            try {
                player.replaceMediaItem(idx, MediaItem.fromUri(
                        mediaUriFor(idx, currentRawUrls.get(idx))));
                player.seekTo(idx, pos);
                player.prepare();
                player.play();
                updateNetworkModeUi();
                showCenterToast(message, 2500);
            } catch (Exception ignore) {}
        });
    }

    private boolean canSwitchNetworkMode(int idx) {
        if (!adFilterOn || idx < 0 || idx >= currentRawUrls.size()
                || proxyBase == null || proxyBase.isEmpty()) return false;
        String raw = currentRawUrls.get(idx).toLowerCase(java.util.Locale.US);
    ...
    private void toggleNetworkMode() {
        ...
        if (!canSwitchNetworkMode(idx)) {
            showCenterToast("当前视频不支持线路切换", 1500);
            return;
        }
    ...
    private void updateNetworkModeUi() {
        int idx = player != null ? player.getCurrentMediaItemIndex() : -1;
        boolean relay = forceRelayIdx.contains(idx) && !forceRawIdx.contains(idx);
    ...
    private String resolveProxyBase(Intent intent) {
        ...
        if (pb == null || pb.isEmpty()) {
            pb = ServiceLocator.INSTANCE.proxyBase();
        }
        return pb == null ? "" : pb;
    }
    ...
    private final class FilterPlaylistParser implements ParsingLoadable.Parser<HlsPlaylist> {
        ...
        public HlsPlaylist parse(Uri uri, java.io.InputStream in) throws IOException {
            ...
            if (adFilterOn) {
                if (!needsClientSideFilter(uri.toString())) {
                    // /m3u8/proxy 已在服务端完成过滤和媒体 URL 重写，避免再同步 POST 一次。
                    filterAttempted = true;
                } else if (proxyBase != null && !proxyBase.isEmpty()) {
    ...
    static boolean needsClientSideFilter(String playlistUrl) {
        return playlistUrl == null
                || !playlistUrl.toLowerCase(java.util.Locale.US).contains("/m3u8/proxy?");
    }
    ...
    private DataSource.Factory buildCacheFactory() {
        ...
        // connect/read 使用 30s 的无数据超时；整次媒体请求不能设总时限，否则长分片/MP4 会在固定时间被中断。
        // followRedirects: 部分 CDN 第一跳 302 到节点服, 默认 OkHttp 是开的但显式声明.
        OkHttpClient http = ServiceLocator.INSTANCE.getMediaClient().newBuilder()
    ...
    private void showSpeedDialog() {
        PlayerChoiceDialog.show(this, "播放速度", SPEED_LABELS, speedIndex, speedButton, i -> {
                    speedIndex = i;
                    if (player != null) {
    ...
    private void bindControlButtons() {
        ...
        speedButton = playerView.findViewById(R.id.btn_speed);
        episodesButton = playerView.findViewById(R.id.btn_episodes);
        sourceButton = playerView.findViewById(R.id.btn_source);
        ...
        if (speedButton != null) speedButton.setOnClickListener(b -> showSpeedDialog());
        if (episodesButton != null) episodesButton.setOnClickListener(b -> showEpisodeDialog());
        if (sourceButton != null) {
            sourceButton.setVisibility(multi ? View.VISIBLE : View.GONE);
            sourceButton.setOnClickListener(b -> showSourceDialog());
        }
        if (networkModeButton != null) {
            networkModeButton.setOnClickListener(b -> toggleNetworkMode());
            updateNetworkModeUi();
        }
    ...
    private void showSourceDialog() {
        ...
        PlayerChoiceDialog.show(this, "切换播放源", names, currentSourceIndex, sourceButton, w -> {
            if (w == currentSourceIndex) return;
            PlaybackTarget target = PlayerSelection.switchSource(
                    w, curEp, curPos, sourceList.get(w).urls.size());
            currentSourceIndex = target.getSourceIndex();
            loadSourceIntoPlayer(
                    target.getSourceIndex(), target.getEpisodeIndex(), target.getPositionMs());
            showCenterToast("已切到「" + sourceList.get(w).name + "」 (从 "
                    + (target.getPositionMs() / 1000) + "s 续播)", 2000);
        });
    }
    ...
    private void showEpisodeSegment(int start, int end, int cur) {
        ...
        PlayerChoiceDialog.show(
                ...
                episodesButton,
                w -> {
                    PlaybackTarget target = PlayerSelection.selectEpisode(
                            currentSourceIndex, start + w, playlistTitles.size());
                    if (player != null) {
                        episodeSwitching = true;
                        player.seekTo(target.getEpisodeIndex(), target.getPositionMs());
                    }
                });
    }
