...
/**
 * 触屏手势助手 — 对齐 web 端语义: 长按临时 2x / 横滑刮擦进度 / 轻扫 ±10s。
 */
public class PlayerGestureHelper {
    ...
    private final Runnable gestureLongPressRunnable = new Runnable() {
        @Override public void run() {
            if (activity.player == null || !activity.player.getPlayWhenReady()
                    || activity.player.getPlaybackState() == Player.STATE_BUFFERING) return;
            gestureMode = 1;
            gestureTempRateBefore = activity.player.getPlaybackParameters().speed;
            activity.player.setPlaybackParameters(new PlaybackParameters(2f));
            activity.showCenterToast("2x 倍速中 ▶▶", GESTURE_HINT_KEEP_MS);
        }
    };

    public PlayerGestureHelper(PlayerActivity activity) {
    ...
    boolean onPlayerTouch(View v, MotionEvent ev) {
        ...
        switch (ev.getActionMasked()) {
            case MotionEvent.ACTION_DOWN: {
                ...
                gestureLastDx = 0;
                activity.playerView.removeCallbacks(gestureLongPressRunnable);
                activity.playerView.postDelayed(gestureLongPressRunnable, GESTURE_LONG_PRESS_MS);
                return true;
            }
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (gestureMode == 0) {
                    if (Math.abs(dx) > GESTURE_SWIPE_TRIGGER_PX && Math.abs(dx) > Math.abs(dy)) {
                        activity.playerView.removeCallbacks(gestureLongPressRunnable);
                        gestureMode = 2;
                        gestureSeekBaseMs = activity.player.getCurrentPosition();
                ...
                gestureLastDx = dx;
                long durMs = activity.player.getDuration();
                int width = Math.max(1, activity.playerView.getWidth());
                ...
                activity.showCenterToast((delta >= 0 ? "快进 " : "快退 ") + fmtGestureMs(Math.abs(delta))
                ...
                if (now - gestureStartTime > GESTURE_FLICK_MS
                    ...
                    activity.player.seekTo(gestureTargetMs);
                }
                return true;
            }
            ...
            case MotionEvent.ACTION_CANCEL: {
                ...
                gestureMode = 0;
                if (mode == 1) {
                    activity.player.setPlaybackParameters(new PlaybackParameters(gestureTempRateBefore));
                    activity.hideGestureToast();
                    return true;
                }
                if (mode == 2) {
                    long dt = System.currentTimeMillis() - gestureStartTime;
                    if (dt < GESTURE_FLICK_MS && Math.abs(gestureLastDx) < GESTURE_FLICK_PX) {
                        long target = activity.player.getCurrentPosition()
                                + (gestureLastDx >= 0 ? 1 : -1) * GESTURE_FLICK_SEEK_SEC * 1000L;
                        long dur = activity.player.getDuration();
                        target = Math.max(0, target);
                        if (dur > 0) target = Math.min(target, dur);
                        activity.player.seekTo(target);
                        activity.showCenterToast((gestureLastDx >= 0 ? "▶ 快进 " : "◀ 快退 ")
                                + GESTURE_FLICK_SEEK_SEC + "s", 800);
                    } else {
                        activity.player.seekTo(gestureTargetMs);
                        activity.showCenterToast("已跳转 " + fmtGestureMs(gestureTargetMs), 800);
                    }
                    return true;
                }
    ...
    void cancelGesture() {
        activity.playerView.removeCallbacks(gestureLongPressRunnable);
        if (gestureMode == 1 && activity.player != null) {
            activity.player.setPlaybackParameters(new PlaybackParameters(gestureTempRateBefore));
        }
        gestureMode = 0;
        activity.hideGestureToast();
