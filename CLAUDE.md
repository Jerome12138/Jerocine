...
- 任何重启线上服务的操作（`docker compose build/up` 等）前先征维护者同意。重部署前若有采集（spider）任务在跑，先 `POST /api/v1/manage/spider/jobs/:id/pause` 记进度再动（重启 jerocine_server 会杀采集 goroutine、丢在途进度）。
...
- **部署命令**：
  - 前端改动：`cd deploy && sudo docker compose build nginx && sudo docker compose up -d --no-deps nginx`
    （`--no-deps` 只重建 nginx、**不连带重启 jerocine_server** → 不打断采集）
  - 后端改动：`... build server && ... up -d server`（**会重启 jerocine_server → 必打断采集**；先 pause 采集）
...
后端 API 改动重建 jerocine_server 后，接口**仍返回旧数据**——因为两层缓存：
1. **nginx proxy_cache**：`deploy/data/nginx/nginx.conf` 对 `/api` 开了 `proxy_cache api_cache; proxy_cache_valid 200 7d`。
   清：`sudo docker exec jerocine_nginx sh -c "rm -rf /var/cache/nginx/api/* && nginx -s reload"`
2. **Redis 应用层缓存**：`v1:movie:detail:<mid>` 等。清需带密码：
   `sudo docker exec jerocine_redis redis-cli -a "$REDIS_PASSWORD" --no-auth-warning --scan --pattern "v1:movie:detail:*" | xargs -r ...del`
